...
└─────────────────┘  │    │    └─────────────────┘
              ...
              │  (Postgres)      │
              │                  │
              │  durable_machines_executors (heartbeats)
...
```sql
INSERT INTO durable_machines_executors (executor_id, last_heartbeat, started_at)
VALUES ($1, NOW(), NOW())
...
orphaned_executors AS (
  ...
  WHERE ws.status = 'PENDING' AND ws.name LIKE 'xstate:%'
    ...
    AND NOT EXISTS (
      SELECT 1 FROM durable_machines_executors xe
