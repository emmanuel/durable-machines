# @xstate-dbos/durable-state-machine
...
```bash
npm install @xstate-dbos/durable-state-machine xstate @dbos-inc/dbos-sdk
```
...
```typescript
import { createDurableMachine, consoleChannel } from "@xstate-dbos/durable-state-machine";
...
|---------|--------|-------------|
| `consoleChannel()` | `@xstate-dbos/durable-state-machine` | In-memory, for testing/development |
| `slackChannel(options)` | `@xstate-dbos/durable-state-machine` | Posts interactive messages to Slack |
| `emailChannel(options)` | `@xstate-dbos/durable-state-machine` | Sends prompt emails via a `sendEmail` callback |
| `twilioSmsChannel(options)` | `@xstate-dbos/durable-state-machine` | Sends prompt SMS via a `sendSms` callback |
...
import { sendMachineEvent, getMachineState } from "@xstate-dbos/durable-state-machine";
