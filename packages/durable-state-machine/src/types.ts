...
/**
 * A handle to a running durable state machine workflow. Provides methods
 * to interact with and observe the machine from outside the workflow.
 *
...
export interface DurableMachineHandle<TContext = Record<string, unknown>> {
  /** The unique DBOS workflow ID for this machine instance. */
...
/** Summary status of a durable machine workflow, typically returned when listing active workflows. */
export interface DurableMachineStatus {
  /** The unique DBOS workflow ID. */
  workflowId: string;
  /** Current DBOS workflow status (e.g. `"PENDING"`, `"SUCCESS"`, `"ERROR"`). */
  status: string;
